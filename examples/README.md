...
| `llama2/` | `mmap` checkpoint load + `Config`/`TransformerWeights` views, `RunState` over one `pageAlloc`'d region, the `forward` pass reusing the `ops.0` kernels, a BPE `tokenizer.0`, a `sampler.0` (argmax + temperature-scaled softmax sampling via a pure-Zero xorshift PRNG), and the autoregressive generation loop in `main.0` that streams decoded tokens to stdout (llama2.c port, v0.1 Phase 7) | `bin/zero build --backend zero-elf64 --emit exe --target linux-musl-x64 examples/llama2 --out .zero/out/llama2` |
