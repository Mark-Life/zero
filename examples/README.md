...
| `llama2/` | full [llama2.c](https://github.com/karpathy/llama2.c) port — `mmap` checkpoint load, the `forward` pass + kernels, BPE tokenizer, argmax/temperature sampling, and the autoregressive generation loop streaming tokens to stdout; matches `llama2.c` token-for-token on `stories15M` (v0.1). See [`examples/llama2/README.md`](./llama2/README.md) | `bin/zero build --backend zero-elf64 --emit exe --target linux-musl-x64 examples/llama2 --out .zero/out/llama2` |
